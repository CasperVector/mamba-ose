...
class PandABlocksClient:
    ...
    def get_blocks_data(self):
        ...
        for block_name in sorted(block_names):
            ...
            for field_name in field_names:
                ...
                elif field_type == "time" or field_subtype == "time":
                    enum_fields.append(field_name + ".UNITS")
            ...
            for field_name in field_names:
                ...
                elif field_name + ".UNITS" in enum_queues:
                    labels = self.recv(enum_queues[field_name + ".UNITS"])
                else:
                    labels = []
    ...
    def get_table_fields(self, block, field):
        ...
        for name, (bits_str, signed) in fields.items():
            ...
            description = strip_ok(self.recv(desc_queues[name])).rstrip(" ")
    ...
    def get_field(self, block, field):
        try:
            ...
            return resp if isinstance(resp, list) else strip_ok(resp)
