...
def fmt_error(e):
    traceback.print_exc()
    return type(e).__name__ or "WtfError", str(e)

@contextlib.contextmanager
def notify_except(errs, notify):
    try:
        yield
    except errs as e:
        notify("err", *fmt_error(e))

def reply_except(errs, reply):
    return notify_except(errs, (lambda _, *rep: reply(*rep)))

def err_state(reply):
    with reply_except(ValueError, reply):
        raise ValueError("request not allowed in this state")
...
class QueueServer(object):
    ...
    def request(self, *req):
        q = queue.Queue()
        self.q.put(((lambda *rep: q.put(list(rep))), list(req)))
        return q.get()
...
class MyItemDelegate(QtWidgets.QStyledItemDelegate):
    ...
    def createEditor(self, parent, option, index):
        ...
        if numpy.issubdtype(typ, numpy.bool_):
            return None
        if not numpy.issubdtype(typ, numpy.number):
